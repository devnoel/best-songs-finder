...
describe("BestSongsFinder", function() {
    ...
    context("given an array 'a' of songs with different qualities", function() {
      context("and a positive number 'n'", function() {
        context("if n <= a.length()", function() {
          it("returns the top rated n songs' titles in descending order of quality", function() {
        ...
        context("if n > a.length()", function() {
          it("returns the songs' titles in descending order of quality", function() {
      ...
      context("and a negative number 'n'", function() {
        context("if abs(n) <= a.length()", function() {
          it("returns the lowest rated abs(n) songs' titles in ascending order of quality", function() {
            const finder = new BestSongsFinder(sampleAlbums.songs, -3)
        ...
        context("if abs(n) > a.length()", function() {
          it("returns the songs' titles in ascending order of quality", function() {
            const finder = new BestSongsFinder(sampleAlbums.songs, -30)
  ...
  describe("useCustomRating(fn)", function() {
    context("given a rating function that expects a song and returns a number", function() {
    ...
    function withFnExpectTitles(fn, titles) {
      BestSongsFinder.useCustomRating(fn)
      const finder = new BestSongsFinder(sampleAlbums.songs, 30)
      expect(finder.find()).to.deep.equal(titles)
    }

    context("given a rating function that also expects the index of the song in the album", function() {
