...
describe("Store", function() {
  ...
  const someIdenticalTitles = Array(6).fill("z")

  describe("save(album)", function() {
    it("throws a TypeError if album is not an array of Song instances", function() {
    ...
    it("returns unique ids", async function() {
      await Promise.all([
        ...
        , expectUniqueIdsAfterSaving(someIdenticalTitles)
  ...
  async function expectLoadedAfterSaving(titles) {
    ...
    const loadedAlbumEntries = await Promise.all( ids.map(id => store.load(id)) )
    const loadedTitles = loadedAlbumEntries.map(albumEntry => albumEntry.album[0].title)
  ...
  async function saveWithTitles(titles) {
    ...
    return await Promise.all( albums.map(album => store.save(album)) )
  }
  ...
  describe("load(id)", function() {
    it("loads the {album, id} object specified by the id", async function() {
      await expectLoadedAfterSaving(titles)
    })
  ...
  describe("loadAll()", function() {
    it("loads all {album, id} pairs", async function() {
  ...
  async function expectLoadedAllAfterSaving(titles) {
    ...
    const loadedTitles = (await store.loadAll()).map(albumEntry => albumEntry.album[0].title)
    expect(loadedTitles).to.have.members(titles)
  }
  ...
  describe("delete(id)", function() {
    ...
    async function expectRemainingFound(remainingIds) {
      const loadedAlbumEntries = await Promise.all( remainingIds.map(id => store.load(id)) )
      expect(loadedAlbumEntries.length).to.equal(remainingIds.length)
    }
